...
describe('respondent1Sequence', () => {
  test('should contain 1 entries in respondent 1 screen sequence', () => {
    ...
    expect(respondentCaseSequence[22].getNextStep({})).toBe(
      '/respondent/upload-document/has-the-court-asked-for-this-documents/:docCategory/:docType'
    );

    expect(respondentCaseSequence[23].url).toBe(
      '/respondent/upload-document/has-the-court-asked-for-this-documents/:docCategory/:docType'
    );
    ...
    ).toBe('/respondent/upload-document/document-sharing-details/otherdocuments/otherdocuments');
    expect(
      respondentCaseSequence[23].getNextStep({ hasCourtAskedForThisDoc: YesOrNo.NO }, {
        params: { docCategory: 'otherdocuments', docType: 'otherdocuments' },
      } as unknown as AppRequest)
    ).toBe('/respondent/upload-document/submit-extra-evidence');
    ...
    expect(
      respondentCaseSequence[24].getNextStep({}, {
        params: { docCategory: 'otherdocuments', docType: 'otherdocuments' },
      } as unknown as AppRequest)
    ).toBe('/respondent/upload-document/sharing-your-documents/otherdocuments/otherdocuments');

    expect(respondentCaseSequence[25].url).toBe(
      '/respondent/upload-document/sharing-your-documents/:docCategory/:docType'
    );
    expect(respondentCaseSequence[25].showInSection).toBe('aboutRespondentCase');
    expect(
      respondentCaseSequence[25].getNextStep({}, {
        params: { docCategory: 'otherdocuments', docType: 'otherdocuments' },
      } as unknown as AppRequest)
    ).toBe('/respondent/upload-document/upload-your-documents/otherdocuments/otherdocuments');
    expect(
      respondentCaseSequence[25].getNextStep({ haveReasonForDocNotToBeShared: YesOrNo.YES }, {
        params: { docCategory: 'otherdocuments', docType: 'otherdocuments' },
      } as unknown as AppRequest)
    ).toBe('/respondent/upload-document/other-party-not-see-this-document/otherdocuments/otherdocuments');

    expect(respondentCaseSequence[26].url).toBe(
      '/respondent/upload-document/other-party-not-see-this-document/:docCategory/:docType'
    );
    expect(respondentCaseSequence[26].showInSection).toBe('aboutRespondentCase');
    expect(
      respondentCaseSequence[26].getNextStep({}, {
        params: { docCategory: 'otherdocuments', docType: 'otherdocuments' },
      } as unknown as AppRequest)
    ).toBe('/respondent/upload-document/upload-your-documents/otherdocuments/otherdocuments');
    ...
    expect(respondentCaseSequence[89].url).toBe('/respondent/yourdocuments/alldocuments/risk_assessment_16a');
    expect(respondentCaseSequence[89].showInSection).toBe('aboutRespondentCase');
    expect(respondentCaseSequence[89].getNextStep({})).toBe('/respondent/yourdocuments/alldocuments/alldocuments');

    expect(respondentCaseSequence[90].url).toBe('/respondent/add-legal-representative');
    ...
    expect(respondentCaseSequence[91].url).toBe('/respondent/remove-legal-representative/start');
    expect(respondentCaseSequence[91].showInSection).toBe('aboutRespondentCase');
    expect(respondentCaseSequence[91].getNextStep({})).toBe('/respondent/remove-legal-representative/confirm');

    expect(respondentCaseSequence[92].url).toBe('/respondent/remove-legal-representative/confirm');
    expect(respondentCaseSequence[92].showInSection).toBe('aboutRespondentCase');
    expect(respondentCaseSequence[92].getNextStep({})).toBe('/respondent/task-list');
