    ...
    if (req.session.userCase?.detailsKnown && req.session.userCase?.startAlternative) {
      if (req.session.userCase.contactDetailsPrivate?.length !== 0) {
        ...
        if (req.session.userCase?.contactDetailsPrivate?.includes('phone')) {
          req.session.userCase.applicant1PhoneNumber = req.session.userCase.applicant1PhoneNumber?.concat('<span class="govuk-hint">' + CONFIDENTIAL_DETAILS.PRIVATE + '</span>');
        }else{
          req.session.userCase.applicant1PhoneNumber = req.session.userCase.applicant1PhoneNumber?.concat('<span class="govuk-hint">' + CONFIDENTIAL_DETAILS.PUBLIC + '</span>');
        }

        if (req.session.userCase?.contactDetailsPrivate?.includes('email')) {
          req.session.userCase.applicant1EmailAddress = req.session.userCase.applicant1EmailAddress?.concat('<span class="govuk-hint">' + CONFIDENTIAL_DETAILS.PRIVATE + '</span>');
        }else{
          req.session.userCase.applicant1EmailAddress = req.session.userCase.applicant1EmailAddress?.concat('<span class="govuk-hint">' + CONFIDENTIAL_DETAILS.PUBLIC + '</span>');
        }
      } else{
        req.session.userCase.applicant1PhoneNumber = req.session.userCase.applicant1PhoneNumber?.concat('<span class="govuk-hint">' + CONFIDENTIAL_DETAILS.PUBLIC + '</span>');
        req.session.userCase.applicant1EmailAddress = req.session.userCase.applicant1EmailAddress?.concat('<span class="govuk-hint">' + CONFIDENTIAL_DETAILS.PUBLIC + '</span>');
