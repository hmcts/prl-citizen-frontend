...
import { C100_OTHER_PROCEEDINGS_DETAILS, C100_OTHER_PROCEEDINGS_DOCUMENT_SUMMARY, C100_OTHER_PROCEEDINGS_DOCUMENT_UPLOAD, C100_OTHER_PROCEEDINGS_ORDER_DETAILS } from '../../urls';
...
describe('OtherProceedingsNavigationController', () => {
  test('From order selection screen -> navigate to the order details screen for capturing the first selected order', async () => {
    const nextUrl = OtherProceedingsNavigationController.getNextUrl(C100_OTHER_PROCEEDINGS_DETAILS, dummyRequest.session.userCase, dummyRequest.query)
  ...
  test('From care order screen -> navigate to the care order document upload screen for orderId 2', async () => {
    ...
    const nextUrl = OtherProceedingsNavigationController.getNextUrl(C100_OTHER_PROCEEDINGS_ORDER_DETAILS, dummyRequest.session.userCase, dummyRequest.query)
  ...
  test('From care order doc uplaod screen for orderId 2 -> navigate to the care order document upload screen for orderId 4', async () => {
    ...
    const nextUrl = OtherProceedingsNavigationController.getNextUrl(C100_OTHER_PROCEEDINGS_DOCUMENT_UPLOAD, dummyRequest.session.userCase, dummyRequest.query)
