import { C100_OTHER_PROCEEDINGS_CURRENT_PREVIOUS, C100_OTHER_PROCEEDINGS_DETAILS, C100_OTHER_PROCEEDINGS_DOCUMENT_SUMMARY, C100_OTHER_PROCEEDINGS_DOCUMENT_UPLOAD, C100_OTHER_PROCEEDINGS_ORDER_DETAILS, PageLink } from "../../urls"
...
class OtherProceedingsNavigationController {
  ...
  private getNextOrderId(): number {
    return this.orders.findIndex((order, index) => order.orderCopy === YesNoEmpty.YES && index > Number(this.orderId) - 1) + 1
  }
private getCurrentOrderTypeIndex():number{
  ...
  private getNextOrderType():C100OrderTypes|''{
    ...
    return currentSelectedOrderIndex<=this.selectedOrderTypes.length-1 ? this.selectedOrderTypes[currentSelectedOrderIndex+1]: ''
  }
  ...
  public getNextUrl(currentPage: PageLink, caseData: Partial<Case>, queryParams?: Record<string, any>): PageLink {
    ...
    switch (currentPage) {
      ...
      case C100_OTHER_PROCEEDINGS_ORDER_DETAILS:
        ...
        if (orderId) { // if any order has order copy to be uploaded
          nextUrl = `${C100_OTHER_PROCEEDINGS_DOCUMENT_UPLOAD}?orderType=${this.orderType}&orderId=${orderId}`
        } else { // none of the orders in the current order type have order copy to be uploaded
          const nextOrderType = this.getNextOrderType()
          if (nextOrderType) {
            nextUrl = `${C100_OTHER_PROCEEDINGS_ORDER_DETAILS}?orderType=${nextOrderType}`
          } else { // there is no other order type present
            if (isAnyOrderWithOrderCopy(caseData?.otherProceedings?.order!)) { // check at last if there were any previous order types having at least an order with order copy
      ...
      case C100_OTHER_PROCEEDINGS_DOCUMENT_UPLOAD:
        ...
        if (nextOrderId) { // if there are any more orders with order copy
