...
export default class AddChildrenPostController extends PostController<AnyObject> {
  ...
  public async post(req: AppRequest<AnyObject>, res: Response): Promise<void> {
    ...
    if (removeChild) {
      req.session.userCase.cd_children = req.session.userCase.cd_children?.filter(child => child.id !== removeChild);
    ...
    if (addChild) {
      ...
      if (req.session.userCase?.cd_children?.length) {
        ...
        if (childrenDetailsErrors.length) {
          ...
          req.session.userCase.cd_children = this.transformData(rest, req.session.userCase.cd_children);
        } else {
          ...
          if (childFirstName && childLastName) {
            ...
            req.session.userCase.cd_children.push({
              ...getDataShape(),
              firstName: childFirstName as string,
              lastName: childLastName as string,
            });
      ...
      const dataToSave = { cd_children: [...this.transformData(rest, req.session.userCase.cd_children)] };

      if (childFirstName && childLastName) {
        dataToSave.cd_children.push({
          ...getDataShape(),
