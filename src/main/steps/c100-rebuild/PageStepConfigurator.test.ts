...
describe('PageStepConfigurator', () => {
  ...
  test('deriveSteps should return appropriate steps according to selectedPageSteps', () => {
    ...
    expect(Object.values(pageSteps)).toHaveLength(2);
    expect(pageSteps).toHaveProperty(
    ...
    expect(pageSteps).not.toHaveProperty(
  ...
  test('getNextPage should return the next pageurl if applicable', () => {
    PageStepConfigurator.deriveSteps(reasonableAdjustmentsConfig.id, ['documentsHelp', 'extraSupport']);
    expect(PageStepConfigurator.getNextPage(reasonableAdjustmentsConfig.id)).toHaveProperty(
      'url',
      '/c100-rebuild/reasonable-adjustments/disability-requirements/document-information'
    );
    expect(
      PageStepConfigurator.getNextPage(
        reasonableAdjustmentsConfig.id,
        '/c100-rebuild/reasonable-adjustments/disability-requirements/document-information'
      )
    ).toHaveProperty('url', '/c100-rebuild/reasonable-adjustments/disability-requirements/support-court');
    expect(
      PageStepConfigurator.getNextPage(
        reasonableAdjustmentsConfig.id,
