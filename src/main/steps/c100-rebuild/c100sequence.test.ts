...
describe('C100Sequence', () => {
  test('should contain 1 entries in c100 screen sequence', () => {
    ...
    expect(C100Sequence[41].url).toBe('/c100-rebuild/submit-application/paper-submit-hwf');
    expect(C100Sequence[41].showInSection).toBe('c100');
    expect(C100Sequence[41].getNextStep({})).toBe('/c100-rebuild/submit-application/paper-submit-hwf');
  });
});
