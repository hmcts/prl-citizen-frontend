...
describe('C100Sequence', () => {
  test('should contain 1 entries in c100 screen sequence', () => {
    expect(C100Sequence).toHaveLength(43);
    ...
    expect(C100Sequence[41].getNextStep({})).toBe('/c100-rebuild/document-submission');

    expect(C100Sequence[42].url).toBe('/c100-rebuild/miam/miam-other-proceedings');
    expect(C100Sequence[42].showInSection).toBe('c100');
    expect(C100Sequence[42].getNextStep({ miamOtherProceedings: YesOrNo.YES })).toBe(
      '/c100-rebuild/confidentiality/details-know'
    );
    expect(C100Sequence[42].getNextStep({ miamOtherProceedings: YesOrNo.NO })).toBe(
      '/c100-rebuild/confidentiality/details-know'
    );
  });
});
