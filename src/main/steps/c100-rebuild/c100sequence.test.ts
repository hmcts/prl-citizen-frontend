...
describe('C100Sequence', () => {
  test('should contain 1 entries in c100 screen sequence', () => {
    expect(C100Sequence).toHaveLength(61);
    ...
    expect(C100Sequence[50].getNextStep({})).toBe('/c100-rebuild/miam/mediator-confirmation');
    ...
    expect(C100Sequence[52].getNextStep({})).toBe('/c100-rebuild/miam/urgency');
    ...
    expect(C100Sequence[55].getNextStep({ miam_validReason: YesOrNo.YES })).toBe('/c100-rebuild/miam/valid-reason');
    expect(C100Sequence[55].getNextStep({ miam_validReason: YesOrNo.NO })).toBe('/c100-rebuild/confidentiality/start');
    ...
    expect(C100Sequence[60].getNextStep({ otherConcernsDrugs: YesOrNo.YES })).toBe(
      '/c100-rebuild/safety-concerns/other-concerns/drugs'
    );
    expect(C100Sequence[60].getNextStep({ otherConcernsDrugs: YesOrNo.NO })).toBe(
