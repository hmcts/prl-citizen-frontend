...
describe('C100Sequence', () => {
  ...
  test('should contain 1 entries in c100 screen sequence', () => {
    ...
    expect(C100Sequence[63].url).toBe('/c100-rebuild/screening-questions/permissions-why/:removeFileId?');
    ...
    expect(
      C100Sequence[64].getNextStep({ sq_courtPermissionRequired: YesOrNo.YES }, {
        session: { applicationSettings: { hasC100ApplicationBeenCompleted: false } },
      } as unknown as AppRequest)
    ...
    expect(
      C100Sequence[65].getNextStep({}, {
        session: { applicationSettings: { hasC100ApplicationBeenCompleted: false } },
      } as unknown as AppRequest)
    ).toBe('/c100-rebuild/screening-questions/contact-representative');

    expect(C100Sequence[66].url).toBe('/c100-rebuild/applicant/add-applicants');
    expect(C100Sequence[66].showInSection).toBe('c100');
    expect(C100Sequence[66].getNextStep(applicantMockRequest.session.userCase, applicantMockRequest)).toBe(
    ...
    expect(C100Sequence[75].url).toBe('/c100-rebuild/child-details/has-other-children');
    expect(C100Sequence[75].showInSection).toBe('c100');
    expect(C100Sequence[75].getNextStep({ ocd_hasOtherChildren: YesOrNo.NO })).toBe(
      '/c100-rebuild/applicant/add-applicants'
    );
    expect(C100Sequence[75].getNextStep({ ocd_hasOtherChildren: YesOrNo.YES })).toBe(
      '/c100-rebuild/child-details/other-children/names'
    );

    expect(C100Sequence[76].url).toBe('/c100-rebuild/child-details/other-children/names');
    ...
    expect(C100Sequence[77].url).toBe('/c100-rebuild/child-details/other-children/:childId/personal-details');
    expect(C100Sequence[77].showInSection).toBe('c100');
    expect(C100Sequence[77].getNextStep(otherChildrenMockData.session.userCase, otherChildrenMockData)).toBe(
      '/c100-rebuild/applicant/add-applicants'
    );

    expect(C100Sequence[78].url).toBe('/c100-rebuild/respondent-details/add-respondents');
    expect(C100Sequence[78].showInSection).toBe('c100');
    expect(C100Sequence[78].getNextStep(respondentMockData.session.userCase)).toBe(
      '/c100-rebuild/respondent-details/2732dd53-2e6c-46f9-88cd-08230e735b08/personal-details'
    );

    expect(C100Sequence[79].url).toBe('/c100-rebuild/respondent-details/:respondentId/personal-details');
    ...
    expect(C100Sequence[82].url).toBe('/c100-rebuild/respondent-details/:respondentId/address/select');
    ...
    expect(C100Sequence[83].url).toBe('/c100-rebuild/respondent-details/:respondentId/address/manual');
    ...
    expect(C100Sequence[86].url).toBe('/c100-rebuild/respondent-details/:respondentId/confidentiality/feedback');
    ...
    expect(C100Sequence[87].url).toBe('/c100-rebuild/respondent-details/:respondentId/confidentiality/feedback-no');
    expect(C100Sequence[87].showInSection).toBe('c100');
    expect(C100Sequence[87].getNextStep(respondentMockData.session.userCase, respondentMockData)).toBe(
      '/c100-rebuild/other-person-details/other-person-check'
    );
    ...
    expect(C100Sequence[91].url).toBe('/c100-rebuild/other-person-details/:otherPersonId/address/lookup');
    expect(C100Sequence[91].showInSection).toBe('c100');
    ...
    expect(C100Sequence[92].url).toBe('/c100-rebuild/other-person-details/:otherPersonId/address/select');
    expect(C100Sequence[92].showInSection).toBe('c100');
    ...
    expect(C100Sequence[93].url).toBe('/c100-rebuild/other-person-details/:otherPersonId/address/manual');
    expect(C100Sequence[93].showInSection).toBe('c100');
    ...
    expect(C100Sequence[94].url).toBe(
      '/c100-rebuild/other-person-details/:otherPersonId/relationship-to-child/:childId'
    );
    expect(C100Sequence[94].showInSection).toBe('c100');
    expect(C100Sequence[94].getNextStep(otherPersonMockData.session.userCase, otherPersonMockData)).toBe(
      '/c100-rebuild/refuge/staying-in-refuge/7228444b-ef3f-4202-a1e7-cdcd2316e1f6?'
    );

    expect(C100Sequence[95].url).toBe('/c100-rebuild/child-details/:childId/live-with/mainly-live-with');
    expect(C100Sequence[95].showInSection).toBe('c100');
    expect(C100Sequence[95].getNextStep(childrenMockData.session.userCase, childrenMockData)).toBe(
      '/c100-rebuild/child-details/7483640e-0817-4ddc-b709-6723f7925474/live-with/living-arrangements'
    );

    expect(C100Sequence[96].url).toBe('/c100-rebuild/child-details/:childId/live-with/living-arrangements');
    expect(C100Sequence[96].showInSection).toBe('c100');
    expect(
      C100Sequence[96].getNextStep({ ...childrenMockData.session.userCase, oprs_otherPersons: [] }, childrenMockData)
    ).toBe('/c100-rebuild/other-proceedings/current-previous-proceedings');

    expect(C100Sequence[97].url).toBe('/c100-rebuild/applicant/:applicantId/personal-details');
    ...
    expect(C100Sequence[98].url).toBe('/c100-rebuild/applicant/:applicantId/relationship-to-child/:childId');
    expect(C100Sequence[98].showInSection).toBe('c100');
    expect(C100Sequence[98].getNextStep(applicantMockRequest.session.userCase, applicantMockRequest)).toBe(
      '/c100-rebuild/applicant/2732dd53-2e6c-46f9-88cd-08230e735b08/relationship-to-child/7483640e-0817-4ddc-b709-6723f7925635'
    );

    expect(C100Sequence[99].url).toBe('/c100-rebuild/applicant/:applicantId/contact-detail');
    ...
    expect(C100Sequence[100].url).toBe('/c100-rebuild/consent-order/upload');
    expect(C100Sequence[100].showInSection).toBe('c100');
    expect(C100Sequence[100].getNextStep({})).toBe('/c100-rebuild/consent-order/upload-confirmation');

    expect(C100Sequence[101].url).toBe('/c100-rebuild/consent-order/upload-confirmation');
    expect(C100Sequence[101].showInSection).toBe('c100');
    expect(
      C100Sequence[101].getNextStep({}, {
        session: { applicationSettings: { hasC100ApplicationBeenCompleted: false } },
      } as unknown as AppRequest)
    ).toBe('/c100-rebuild/hearing-urgency/urgent');
    ...
    expect(C100Sequence[102].url).toBe('/c100-rebuild/check-your-answers');
    expect(C100Sequence[102].showInSection).toBe('c100');
    expect(
      C100Sequence[102].getNextStep({}, {
        session: { applicationSettings: { hasC100ApplicationBeenCompleted: false } },
      } as unknown as AppRequest)
    ).toBe('/c100-rebuild/check-your-answers');

    expect(C100Sequence[103].url).toBe('/c100-rebuild/applicant/:applicantId/contact-preference');
    ...
    expect(C100Sequence[104].url).toBe('/c100-rebuild/:caseId/withdraw');
    expect(C100Sequence[104].showInSection).toBe('c100');
    expect(C100Sequence[104].getNextStep({})).toBe('/task-list/applicant');

    expect(C100Sequence[105].url).toBe('/c100-rebuild/withdraw/confirmation');
    expect(C100Sequence[105].showInSection).toBe('c100');
    expect(C100Sequence[105].getNextStep({})).toBe('/');

    expect(C100Sequence[106].url).toBe('/c100-rebuild/other-person-details/:otherPersonId/confidentiality');
    expect(C100Sequence[106].showInSection).toBe('c100');
    expect(C100Sequence[106].getNextStep(otherPersonMockData.session.userCase, otherPersonMockData)).toBe(
