...
describe('C100Sequence', () => {
  test('should contain 1 entries in c100 screen sequence', () => {
    ...
    expect(C100Sequence[36].url).toBe('/c100-rebuild/safety-concerns/concerns-for-safety');
    expect(C100Sequence[36].showInSection).toBe('c100');
    expect(C100Sequence[36].getNextStep({})).toBe('/c100-rebuild/confidentiality/details-know');
  });
});
