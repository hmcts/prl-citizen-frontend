...
describe('C100Sequence', () => {
  test('should contain 1 entries in c100 screen sequence', () => {
    ...
    expect(C100Sequence[48].url).toBe('/c100-rebuild/safety-concerns/applicant/report-abuse');
    expect(C100Sequence[48].showInSection).toBe('c100');
    expect(C100Sequence[48].getNextStep({})).toBe('/c100-rebuild/safety-concerns/applicant/report-abuse');
  });
});
