...
describe('C100Sequence', () => {
  test('should contain 1 entries in c100 screen sequence', () => {
    expect(C100Sequence).toHaveLength(38);
    ...
    expect(C100Sequence[36].getNextStep({})).toBe('/c100-rebuild/other-proceedings/current-previous-proceedings');

    expect(C100Sequence[37].url).toBe('/c100-rebuild/safety-concerns/concern-about');
    expect(C100Sequence[37].showInSection).toBe('c100');
    expect(C100Sequence[37].getNextStep({})).toBe('/c100-rebuild/confidentiality/details-know');
  });
});
