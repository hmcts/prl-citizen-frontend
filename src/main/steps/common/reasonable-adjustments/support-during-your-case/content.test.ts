...
describe('Disability requirements content', () => {
  ...
  test('should contain disabilityRequirements field', () => {
    ...
    const assistanceRequiredField = disabilityRequirementsField.values[0].subFields
      ?.ra_disabilityRequirements_subfield as FormOptions;

    expect(disabilityRequirementsField.labelHidden).toBe(true);
    expect(disabilityRequirementsField.type).toBe('radios');
    expect(disabilityRequirementsField.classes).toBe('govuk-radios');
    expect((disabilityRequirementsField.label as Function)(generatedContent)).toBe(en.headingTitle);

    (disabilityRequirementsField.validator as Validator)(generatedContent);
    expect(isFieldFilledIn).toHaveBeenCalled();

    expect((disabilityRequirementsField.values[0].label as Function)(commonContentEN)).toBe(YesOrNo.YES);
    expect((disabilityRequirementsField.values[1].label as Function)(commonContentEN)).toBe(YesOrNo.NO);
