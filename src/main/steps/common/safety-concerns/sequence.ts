...
import {
  ...
  C1A_SAFETY_CONCERNS_ABDUCTION_PASSPORT_AMOUNT,
  C1A_SAFETY_CONCERNS_ABDUCTION_PASSPORT_OFFICE,
  C1A_SAFETY_CONCERNS_ABDUCTION_PASSPORT_OFFICE_NOTIFICATION,
  C1A_SAFETY_CONCERNS_CONCERNS_ABOUT_CHILD,
  C1A_SAFETY_CONCERNS_CONCERNS_ABOUT_YOURSELF,
  C1A_SAFETY_CONCERNS_CONCERNS_FOR_SAFETY,
  C1A_SAFETY_CONCERNS_CONCERN_ABOUT,
  C1A_SAFETY_CONCERNS_CONCERN_GUIDANCE,
  C1A_SAFETY_CONCERNS_NOFEEDBACK,
  C1A_SAFETY_CONCERNS_ORDERS_REQUIRED_COURT_ACTION,
  C1A_SAFETY_CONCERNS_ORDERS_REQUIRED_UNSUPERVISED,
  C1A_SAFETY_CONCERNS_OTHER,
    ...
    return [
      ...
      },
      {
        ...
        getNextStep: (caseData, req) => {
          const C100rebuildJourney = req?.originalUrl?.startsWith(C100_URL);
          return C100rebuildJourney
            ? (applyParms(C1A_SAFETY_CONCERNS_CONCERNS_FOR_SAFETY, { root: RootContext.C100_REBUILD }) as PageLink)
      ...
      },
      {
        ...
        getNextStep: (caseData, req) => {
          const C100rebuildJourney = req?.originalUrl?.startsWith(C100_URL);
          return C100rebuildJourney
            ? (applyParms(C1A_SAFETY_CONCERNS_OTHER, { root: RootContext.C100_REBUILD }) as PageLink)
            : (applyParms(C1A_SAFETY_CONCERNS_OTHER, { root: RootContext.RESPONDENT }) as PageLink);
        },
      },
      {
        ...
        getNextStep: (caseData, req) => {
          const C100rebuildJourney = req?.originalUrl?.startsWith(C100_URL);
          return C100rebuildJourney
            ? (applyParms(C1A_SAFETY_CONCERNS_ABDUCTION_PASSPORT_OFFICE_NOTIFICATION, {
                root: RootContext.C100_REBUILD,
              }) as PageLink)
            : (applyParms(C1A_SAFETY_CONCERNS_ABDUCTION_PASSPORT_OFFICE_NOTIFICATION, {
      ...
      },
      {
        ...
        getNextStep: (caseData, req) => {
          const C100rebuildJourney = req?.originalUrl?.startsWith(C100_URL);
          return C100rebuildJourney
            ? (applyParms(C1A_SAFETY_CONCERNS_ORDERS_REQUIRED_COURT_ACTION, {
                root: RootContext.C100_REBUILD,
              }) as PageLink)
            : (applyParms(C1A_SAFETY_CONCERNS_ORDERS_REQUIRED_COURT_ACTION, {
                root: RootContext.RESPONDENT,
      ...
      },
      {
        ...
        getNextStep: (caseData, req) => {
          const C100rebuildJourney = req?.originalUrl?.startsWith(C100_URL);
          return C100rebuildJourney
            ? (applyParms(C1A_SAFETY_CONCERNS_ORDERS_REQUIRED_UNSUPERVISED, {
                root: RootContext.C100_REBUILD,
              }) as PageLink)
            : (applyParms(C1A_SAFETY_CONCERNS_ORDERS_REQUIRED_UNSUPERVISED, {
                root: RootContext.RESPONDENT,
      ...
      },
      {
        ...
        getNextStep: (caseData, req) => {
          const C100rebuildJourney = req?.originalUrl?.startsWith(C100_URL);
          return C100rebuildJourney
            ? (applyParms(C1A_SAFETY_CONCERNS_ABDUCTION_PASSPORT_OFFICE, {
                root: RootContext.C100_REBUILD,
              }) as PageLink)
      ...
      },
      {
        ...
        getNextStep: (data, req) => {
          ...
          if (data.c1A_passportOffice === YesOrNo.YES) {
            return C100rebuildJourney
              ? (applyParms(C1A_SAFETY_CONCERNS_ABDUCTION_PASSPORT_AMOUNT, {
                  root: RootContext.C100_REBUILD,
                }) as PageLink)
              : (applyParms(C1A_SAFETY_CONCERNS_ABDUCTION_PASSPORT_AMOUNT, {
                  root: RootContext.RESPONDENT,
