...
describe('applicationWithinProceedingsSequence', () => {
  ...
  test('should contain 1 entries in applicationWithinProceedingsSequence 1 screen sequence', () => {
    ...
    expect(applicationWithinProceedingsSequence[11].getNextStep(userCase, req)).toBe(
      '/application-within-proceedings/C2/delay-or-cancel-hearing-date/application-submitted'
    );
    expect(
        ...
        session: {
          ...
          applicationSettings: {
            ...
            awpSelectedApplicationDetails: {
              ...req.session.applicationSettings.awpSelectedApplicationDetails,
              applicationFee: '£20',
            },
          },
        },
    ...
    ).toBe('/application-within-proceedings/C2/delay-or-cancel-hearing-date/application-submitted');
    ...
    expect(applicationWithinProceedingsSequence[12].getNextStep({ awp_hasSupportingDocuments: YesOrNo.YES }, req)).toBe(
      '/application-within-proceedings/C2/delay-or-cancel-hearing-date/supporting-document-upload'
    );
    expect(applicationWithinProceedingsSequence[12].getNextStep({ awp_hasSupportingDocuments: YesOrNo.NO }, req)).toBe(
