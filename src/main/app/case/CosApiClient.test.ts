...
describe('CosApiClient', () => {
  ...
  test('generateC7Document', async () => {
    ...
    const client = new CosApiClient('abc', mockLogger);
    const actual = await client.generateC7DraftDocument('123456', '123456789', false);
  ...
  test('generateC7Document throws exception', async () => {
    ...
    let flag = false;
    try {
      await client.generateC7DraftDocument('123456', '123456789', false);
    } catch (error) {
      flag = true;
    }
  ...
  test('findCourtByPostCodeAndService throws error', async () => {
    mockedAxios.post.mockRejectedValueOnce;
    const client = new CosApiClient('abc', mockLogger);
    let flag = false;
    try {
      await client.findCourtByPostCodeAndService('SA1 3TU');
    } catch (error) {
      flag = true;
    }
    expect(flag).toEqual(true);
  ...
  test('findOsCourtByPostCodeAndService throws error', async () => {
    mockedAxios.post.mockRejectedValueOnce;
    const client = new CosApiClient('abc', mockLogger);
    const req = mockRequest();
    let flag = false;
    try {
      await client.findOsCourtByPostCodeAndService('SA1 3TU', req.session.user);
    } catch (error) {
      flag = true;
    }
    expect(flag).toEqual(true);
